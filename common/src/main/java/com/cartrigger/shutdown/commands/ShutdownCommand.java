...
public class ShutdownCommand {
    ...
    private static int execute(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        try {
            ...
            ShutdownMod.LOGGER.info("Shutdown command executed by: " + source.getTextName());
            ...
            new Thread(() -> {
                try {
                    ...
                    ShutdownMod.LOGGER.error("Failed to execute shutdown command: " + e.getMessage());
                }
            }).start();
            
        } catch (Exception e) {
            source.sendFailure(Component.literal("Failed to execute shutdown: " + e.getMessage()));
            ShutdownMod.LOGGER.error("Error in shutdown command: " + e.getMessage());
            return 0;
        }
